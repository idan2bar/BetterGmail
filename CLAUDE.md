...
**Gmail API / auth constraints** – uses the restricted scope `gmail.metadata` only (headers, no bodies). That scope
forbids the `q` search parameter, so the API is only ever used to list by label; searching is done through Gmail's own
UI via the SDK router. The manifest carries `key` (stable extension id), `oauth2.client_id` and `oauth2.scopes`,
